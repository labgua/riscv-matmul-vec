        ...
        try:
            with open(filename, 'r') as f:
                ...
                size_col = next((col for col in fieldnames 
                                if col.lower() in['input size', 'size']), None)
                ...
                loads_col = next((col for col in fieldnames 
                                 if col.lower() in ['l1-dcache-loads', 'l1d-load']), None)
                misses_col = next((col for col in fieldnames 
                                  if col.lower() in ['l1-dcache-load-misses', 'l1d-misses']), None)
                miss_rate_col = next((col for col in fieldnames 
                                     if col.lower() in ['cache miss rate', 'cachemiss-rate']), None)
                time_col = next((col for col in fieldnames 
                                if col.lower() in ['time', 'tempo di esecuzione', 'exec time']), None)

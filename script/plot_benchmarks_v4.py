...
# MODIFICA 1: regex aggiornata per catturare LMUL negativi (-?[0-9]+)
record_re = re.compile(
    r"BENCHMARK_RECORD\s*:\s*([A-Za-z0-9_]+),\s*([0-9.]+),\s*([0-9]+)"
    r"(?:,\s*([0-9]+))?"   # kernel (opzionale)
    r"(?:,\s*(-?[0-9]+))?"   # lmul (opzionale, CON SEGNO MENO)
)
...
for rec in record_re.finditer(content):
    ...
    lmul = rec.group(5) if rec.group(5) else "N/A"  # ora può essere "-2", "-4", ecc.
...
# MODIFICA 2: gestione LMUL frazionali nella formattazione
...
for version_name, runs_in_version in sorted(version_groups.items()):
    ...
    for idx, config in enumerate(sorted_configs):
        ...
        # MODIFICA 3: formatta LMUL frazionale anche nelle legende per-versione
        try:
            lmul_val = int(lmul)
            if lmul_val < 0:
                frac_map = {-2: "1/2", -4: "1/4", -8: "1/8"}
...
# ==================== NUOVA SEZIONE: GRAFICI A BARRE PER (VERSIONE × SIZE) ====================
